...
class Deck:                                       ##instance 없이 쓰는 static method로 Deck.make_deck()
    ...
    def pop_card(self):
        ...
        received_card = []
        for i in range(1,3):
            card = self.shuffled_deck.pop(i)
            received_card.append(card)
        print(received_card, len(self.shuffled_deck))
...
class Player(Deck):
    def hit(self, player_deck):
                ## hit 하는거                
        player_deck.append(self.shuffled_deck.pop(0))
...
class Dealer(Deck):
    def dealer_auto_hit(self, dealer_deck):
        dealer_deck.append(self.shuffled_deck.pop(0))
